...
- Integración de datos de otros registros administracíon.
...
### Informe Viviendas Rural por Provincia

```{r parajes, warning=FALSE}
campos <- dbListFields(con, "public.viviendas_segrural")

print("Campos de tabla public.viviendas_segrural")
knitr::kable(campos, format = "simple", caption = "Campos de tabla paraje")

df <- dbGetQuery(con, "SELECT prov, sum(vivs) viviendas
from (
SELECT  prov, dpto, codloc, frac, radio, 
  max(CASE when cantviv = '' then 0::integer else cantviv::integer end::integer) vivs, count(*) registros
	FROM public.viviendas_segrural
  where not (prov ilike '(%')
  GROUP BY prov, dpto, codloc, frac, radio
  ORDER BY count(*) desc) foo
    GROUP BY prov 
  ORDER BY prov asc;")
knitr::kable(df, caption = "Tabla paraje", digits = 2, longtable = TRUE) %>%
  kable_styling(font_size = 8) %>%
